...
const INITIAL_ACHIEVEMENTS: Achievement[] = [
  ...
  { id: 'ach-8', title: 'Gourmet Logger', description: 'Record 10 meals in your Food tracker', icon: 'Apple', category: 'food', xpReward: 150, progress: 3, maxProgress: 10 }
];
...
const INITIAL_MEALS: Meal[] = [
  { id: 'm-1', userId: 'user-1', name: 'Egg White Omelette & Avocado Toast', calories: 420, protein: 28, carbs: 32, fat: 18, type: 'Breakfast', date: new Date().toISOString().split('T')[0] },
  { id: 'm-2', userId: 'user-1', name: 'Grilled Chicken Breast with Brown Rice', calories: 650, protein: 48, carbs: 65, fat: 10, type: 'Lunch', date: new Date().toISOString().split('T')[0] },
  { id: 'm-3', userId: 'user-1', name: 'Post-Workout Whey Protein Shake', calories: 220, protein: 30, carbs: 5, fat: 2, type: 'Snack', date: new Date().toISOString().split('T')[0] }
];

const INITIAL_SCHEDULE: WorkoutSchedule = {
  days: ['Mon', 'Wed', 'Fri'],
  reminderTime: '07:30'
};
...
class GymeoDatabase {
  ...
  private load() {
    try {
      ...
      if (serialized) {
        ...
        // Fix missing meals or schedule in old storage states
        if (!this.state!.meals) this.state!.meals = [];
        if (!this.state!.schedule) this.state!.schedule = { ...INITIAL_SCHEDULE };
        if (!this.state!.profile.units) this.state!.profile.units = 'metric';
  ...
  private checkStreakIntegrity() {
    ...
    const activitiesOnDate = (date: string) => {
      ...
      const hasFood = this.state!.meals.some(m => m.date === date);
      return hasWorkout || hasFocus || hasWater || hasHabit || hasFood;
    };
  ...
  public registerUser(username: string, displayName: string): UserProfile {
    this.state = {
      ...
      achievements: [
        ...
        { id: 'ach-8', title: 'Gourmet Logger', description: 'Record 10 meals in your Food tracker', icon: 'Apple', category: 'food', xpReward: 150, progress: 0, maxProgress: 10 }
      ],
      friends: [...INITIAL_FRIENDS],
      activities: [...INITIAL_ACTIVITIES],
      weightHistory: [],
  ...
  public toggleHabit(id: string, date: string): { habit: Habit; completed: boolean } {
    ...
    if (index >= 0) {
      ...
      if (habit.name.toLowerCase().includes('steps') || habit.name.toLowerCase().includes('walk')) {
        this.updateAchievementProgress('walk', 10000);
      }
    }
    this.save();
  ...
  public addMeal(meal: Omit<Meal, 'id' | 'userId'>): Meal {
    this.load();
    const newMeal: Meal = {
      ...meal,
      id: `m-${Date.now()}`,
    ...
    // Add XP for logging food (+20 XP)
    this.addXP(20);
    this.checkAndIncrementStreak(meal.date);
    this.updateAchievementProgress('food', this.state!.meals.length);

    this.addFriendActivity({
      activityType: 'food',
      activityDetail: `logged a meal: "${meal.name}" (${meal.calories} kcal, ${meal.protein}g Protein) 🍳`,
      xpEarned: 20
    });
  ...
  // --- NEW WORKOUT PLANNER METHODS ---
  public getWorkoutSchedule(): WorkoutSchedule {
    this.load();
    return this.state!.schedule || { days: ['Mon', 'Wed', 'Fri'], reminderTime: '07:30' };
  }

  public saveWorkoutSchedule(schedule: WorkoutSchedule): void {
    this.load();
  ...
  private checkAndIncrementStreak(date: string) {
    ...
    const activitiesOnDate = (d: string) => {
      const hasWorkout = this.state!.workouts.some(w => w.date === d && w.id !== `w-${Date.now()}`);
      ...
      const hasFood = this.state!.meals.some(m => m.date === d);
      return hasWorkout || hasFocus || hasWater || hasHabit || hasFood;
    };

    const alreadyCompletedGoalToday = activitiesOnDate(today);
    if (alreadyCompletedGoalToday) return;
